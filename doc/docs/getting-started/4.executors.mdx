...
```dart
class DartzEitherAdapter implements EitherAdapter<Exception, String> {
  ...
  @override
  dynamic fold(Function(Exception l) leftF, Function(String r) rightF){
...
```dart
 @override
  Future<void> fetchData(){
   executeEither(() => DartzEitherAdapter.adapter(myUsecase()));
  }
```